...
// endpoint that returns eligible results is preferred on later calls. Only uploads
// from configured movie/show users are eligible for automatic downloads.
type Params struct {
...
type X1337 struct {
	...
	urls       []string
	movieUsers map[string]struct{}
	showUsers  map[string]struct{}
	preferred  int
	mu         sync.Mutex
	log        *slog.Logger
	configured bool
...
func (x *X1337) InitWithParams(params *Params) error {
	...
	x.urls = urls
	x.movieUsers = userSet(params.MovieUsers)
	x.showUsers = userSet(params.ShowUsers)
	x.Timeout = timeout
	if x.Client == nil {
...
func (x *X1337) GetTorrents(ctx context.Context, value any) error {
	...
	eligible := func(result searchResult) bool {
		if _, ok := users[result.User]; !ok {
			return false
		}
		guess := whatsthis.Video(strings.ReplaceAll(result.Name, " ", "."))
		quality := polochon.Quality(guess.ScreenSize)
		return accept(guess) && quality.IsAllowed()
	}
	results, err := x.search(ctx, query, eligible)
	if err != nil {
		return err
	}
	...
	for _, result := range results {
		guess := whatsthis.Video(strings.ReplaceAll(result.Name, " ", "."))
		torrents = append(torrents, result.polochonTorrent(polochon.Quality(guess.ScreenSize)))
	}
...
func (x *X1337) SearchTorrents(query string) ([]*polochon.Torrent, error) {
	results, err := x.search(context.Background(), query, nil)
	if err != nil {
		return nil, err
	}
...
// A backend is preferred only after returning at least one eligible magnet link.
func (x *X1337) search(ctx context.Context, query string, eligible func(searchResult) bool) ([]searchResult, error) {
	...
	for _, endpoint := range x.endpoints() {
		...
		results, err := x.searchEndpoint(requestCtx, endpoint, query, eligible)
		cancel()
		if err == nil {
			if len(results) == 0 {
				empty = true
...
func (x *X1337) searchEndpoint(ctx context.Context, endpoint, query string, eligible func(searchResult) bool) ([]searchResult, error) {
	...
	if eligible != nil {
		results = slices.DeleteFunc(results, func(result searchResult) bool { return !eligible(result) })
	}
	if len(results) == 0 {
		return results, nil
	}
...
func userSet(users []string) map[string]struct{} {
	result := make(map[string]struct{}, len(users))
